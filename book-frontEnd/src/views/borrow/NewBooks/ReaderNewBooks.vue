...
const fetchNewBooks = async () => {
  try {
    ...
    // 构建查询参数 - 重点：只获取近一个月的数据
    const params: any = {
      currentPage: pagination.current,
      pageSize: pagination.pageSize,
      // 只获取一个月内的新书
      startShelfTime: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    };
    ...
    if (response.code === 200) {
      ...
      if (data && data.records && data.records.length > 0) {
        // 转换数据并过滤掉未发布状态的书籍
        const filteredBooks = data.records
          .map(convertBookData)
          .filter((book: Book) => !['未发布', '已借阅', '已预约'].includes(book.status));
