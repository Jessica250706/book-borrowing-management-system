<template>
  <div class="book-new-recommend-page">
    <!-- 页面标题 -->
          ...
          <div class="book-content-section">
            ...
            <div class="book-header">
              ...
              <div class="book-actions">
                <button class="more-btn" @click="toggleMenu(book.id)">⋯</button>
                <div v-if="showMenuId === book.id" class="dropdown-menu">
                  <div class="menu-item" @click="handleDetail(book)">详情</div>
                  <div
                    v-if="
                      book.status === '可借阅' && !book.isBorrowedByCurrentUser
                    "
                    class="menu-item"
                  ...
                  <div
                    v-if="
                      ['待上架', '已借光'].includes(book.status) &&
                      !book.isReservedByCurrentUser
                    "
                    class="menu-item"
            ...
            <div class="book-status-row">
              <span class="status-badge" :class="getStatusClass(book.status)">{{
                book.status
              }}</span>
              <span class="shelf-time"
                >上架时间: {{ formatDate(book.shelfTime) }}</span
              >
            </div>
...
// 转换后端数据为前端格式
const convertBookData = (bookData: any): Book => {
  // 根据用户是否预约/借阅决定显示状态
  let status = BookStatusMap[bookData.bookStatus || 0] || "未知状态";

  // 处理封面图片
  const bookImg =
    bookData.coverUrl && bookData.coverUrl.trim() !== ""
      ? bookData.coverUrl
      : "";

  // 获取分类名称 - 优先使用 categoryName，然后是 category
  const rawCategory = bookData.categoryName || bookData.category;
...
const fetchNewBooks = async () => {
  try {
    ...
    // 添加筛选条件
    // 1. 书籍名称搜索 - 使用 keyword 参数
    if (searchParams.bookName && searchParams.bookName.trim()) {
      params.keyword = searchParams.bookName.trim(); // 使用 keyword 而不是 bookName
    }
    ...
    console.log("新书推荐API请求参数:", params);
    ...
    if (response.code === 200) {
      ...
      if (data && data.records && data.records.length > 0) {
        // 转换数据
        const allBooks = data.records.map(convertBookData);

        const filteredBooks = allBooks.filter(
          (book: Book) => !["未发布"].includes(book.status) // 只过滤未发布状态
        );
